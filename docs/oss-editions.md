...
| 主なドキュメント | [getting-started](getting-started.md) / [#155 demo] | [getting-started](getting-started.md) / [system-architecture](system-architecture.md) | [oss-production-deployment](oss-production-deployment.md) + 運用 Runbook 群 |
...
- **認証は Keycloak に統一（#161 案B, #226）**: API も Web も JWT 必須で、「curl は素通りなのに画面は
  ...
  `testoperator`/`testpass` が投入済み。curl は [getting-started](getting-started.md) のトークン取得手順。
...
- [getting-started.md](getting-started.md) / [system-architecture.md](system-architecture.md) /
