...
| 既定値（settings） | `SettingsRegistry`（#148）: `telemetry.staleThresholdSeconds`（300）＝周期未設定時の既定閾値、`telemetry.staleIntervalMultiplier`（3）＝倍率 N |

> **フォローアップ**: 全ロールで live な `telemetry.*` 設定を反映するには非管理者でも読める設定サーフェスが必要
> （`GET /api/system/settings` は admin 限定）。現状はレジストリ既定値をフロント定数
> （`DEFAULT_STALE_THRESHOLD_SECONDS` / `DEFAULT_STALE_INTERVAL_MULTIPLIER`）で鏡写しにし、上書きの
> live 反映は #148/#176 のフォローアップとする。device / gateway 既定周期の Twin モデル化も後続。
