...
[system-architecture.md](system-architecture.md) を参照してください。
...
鮮度モデルの詳細は [oss-sla-freshness.md](oss-sla-freshness.md)、階層の設計は
[oss-tier-architecture.md](oss-tier-architecture.md) を参照してください。
...
- **SBCO** — Building OS が採用するビル設備の**オントロジー（語彙）**。建物・フロア・空間・機器・
  ...
  として定義します。Brick / REC / IFC / DTDL 標準との対応は [standard-mapping.md](standard-mapping.md)。
...
- 🏛️ [System Architecture（全体像）](system-architecture.md)
