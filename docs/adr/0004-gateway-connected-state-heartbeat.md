...
- **Status**: Accepted — **Phase 1（connected/disconnected）+ Phase 2b（PointList 同期状態, オプション A）
  実装済み**。Phase 2b は `EgressUp.GatewayStatus{applied_revision}` proto 追加 + nexus-gateway の追従で完了。
...
## Implementation notes (Phase 2b — PointList sync state, option A)

- Proto: `EgressUp` gains a third oneof case `GatewayStatus status = 3` (`applied_revision`). Named
  `GatewayStatus`, not `Status`, to avoid clashing with `Grpc.Core.Status` in the generated C# service
  code. Additive/backward-compatible: an old bridge ignores the case, an old gateway never sends it. Kept
  identical in both repos (`proto/gateway_egress.proto`); nexus-gateway regenerates its Go via buf.
- Store: `GatewayConnectionStatus` gains `AppliedRevision` (null until reported);
  `IGatewayConnectionStatusStore.MarkConnectedAsync` gains an optional `appliedRevision`. Still **one
  writer** (GatewayBridge) — the ApiServer pointlist controller is not a second writer (option B was
  rejected for exactly that dual-writer coupling and because `If-None-Match` records the *pre-apply*
  ETag, so it lags every change by one poll).
- Writer (GatewayBridge): `GatewayEgressService` caches the gateway's reported `applied_revision` in a
  `volatile` box; the heartbeat loop carries it on every tick (so a heartbeat never clobbers it) and a
  `Status` frame also refreshes the entry immediately.
- Gateway (nexus-gateway): `pointsync.Loop.AppliedRevision()` exposes the applied ETag; `egress.Agent`
  reports it via `EgressUp.GatewayStatus` right after Hello and every `statusReportInterval` (20s),
  serialized with control-result sends on one mutex.
- Reader (ApiServer): `GatewaysController` compares `AppliedRevision` to the twin-authoritative
  `PointListEtag.Compute` → `GatewayAdminView.PointlistSynced` tri-state (`true`/`false`/`null`).
- UI: `pointlistSyncedLabel` / `pointlistSyncedTone` + a badge in the operator-home panel and
  `/admin/gateways`, kept on a distinct axis from connected and last-seen.
- Verified offline: bridge unit (Status → applied revision), controller unit (tri-state), gateway Go
  unit + race (`AppliedRevision` tracking, Status reporting, no-provider = no Status), web-client vitest.
  Cross-replica KV round-trip of `AppliedRevision` is covered by the Testcontainers store test (weekly).
...
3. ~~**オプション A vs B の採否**（proto 追加を待つか、`If-None-Match` 記録の暫定 B を先に出すか）。~~
   → **決定: A を採用**（Phase 2b で実装済み）。B は (1) ApiServer pointlist controller が heartbeat とは別の
   KV 書き手になり結合が増える、(2) `If-None-Match` は 200 応答時に *適用前* の ETag を記録するため変更ごとに
   1 ポール分ラグる、の 2 点で劣るため不採用。A は「適用済み」ETag を egress の単一書き手経路で運ぶ。
...
- `docs/project/oss-egress-gateway-bridge-plan.md`, `docs/architecture/oss-gateway-pointlist-sync.md`
