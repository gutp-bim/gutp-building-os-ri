...
> ℹ️ OSS スタック（compose）は `building-os.api` を既にコンテナで起動しています（ポート 5000）。
> 上記の `dotnet run` はホストで API を回して開発ループを速くしたいとき用です（compose 版と
> どちらか一方で十分）。
...
| **3000** | **Web Client**（`/admin`, `/resources`, `/admin/twin`） | Building OS | （ホスト `yarn dev`） |
