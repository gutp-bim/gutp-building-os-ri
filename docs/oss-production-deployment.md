...
ローカル起動は [getting-started.md](getting-started.md)、ゲートウェイ接続の契約は
[gateway-integration.md](gateway-integration.md)、設計詳細は [system-architecture.md](system-architecture.md) を参照。
...
（[gateway-integration.md §4](gateway-integration.md)、ingress テレメトリ経路の gateway_id 束縛強化は
...
- **耐障害性**: テレメトリは store-and-forward（NATS JetStream の at-least-once + ParquetLakeWriter durable consumer）。
  ConnectorWorker 停止→再起動でも復旧後 publish で欠落 0（E8 実測 RTO ~4.5s / loss 0、[evaluation-summary.md](evaluation-summary.md)）。
...
- [getting-started.md](getting-started.md) — ローカル起動
- [gateway-integration.md](gateway-integration.md) — ゲートウェイ接続・point list 同期・mTLS
...
- [oss-tier-architecture.md](oss-tier-architecture.md) — Hot/Warm/Cold + Query Router
- [system-architecture.md](system-architecture.md) — 全体設計
