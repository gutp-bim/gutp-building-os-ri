...
（評価軸 E5、[evaluation-summary.md](evaluation-summary.md)）。
...
- 詳細な信頼境界・Traefik 配線は [oss-gateway-bridge-infra.md](oss-gateway-bridge-infra.md) と
  [oss-gateway-pointlist-sync.md](oss-gateway-pointlist-sync.md)。
...
[evaluation-summary.md](evaluation-summary.md)（E1/E5/E6）。

---

## 6. さらに詳しく

- [oss-gateway-pointlist-sync.md](oss-gateway-pointlist-sync.md) — point list 同期 API（ETag/差分/push、mTLS）
- [oss-egress-gateway-bridge-plan.md](oss-egress-gateway-bridge-plan.md) — egress 制御プレーン設計
- [gateway-bridge-ingress-egress-split.md](gateway-bridge-ingress-egress-split.md) — ingress/egress 分離
- [oss-gateway-bridge-infra.md](oss-gateway-bridge-infra.md) — Traefik/mTLS インフラ配線
- [oss-gateway-security-ops.md](oss-gateway-security-ops.md) — 証明書発行/ローテーション/失効・gateway_id 束縛・enforce 段階導入
