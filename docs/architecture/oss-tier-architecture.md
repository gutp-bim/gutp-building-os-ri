...
本アーキテクチャは [`docs/architecture/standard-mapping.md`](standard-mapping.md) の SBCO（`sbco:`）オントロジーに基づく
...
- `docs/architecture/oss-timescaledb-schema.md` — Hypertable スキーマ・圧縮・continuous aggregate の詳細
- `docs/operations/observability-baseline.md` — Prometheus/Loki/Tempo の cardinality ポリシーと retention
- `docs/architecture/standard-mapping.md` — SBCO（`sbco:`）ontology と外部標準語彙の対応表（`bos:` は ControlSchema 拡張）
