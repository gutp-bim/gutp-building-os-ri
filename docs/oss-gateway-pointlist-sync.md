...
> 関連: #181（point list 共有）、`docs/gateway-bridge-ingress-egress-split.md`、
> `docs/oss-gateway-bridge-infra.md`（mTLS ingress）。整合性の計測は E5（#243）。
...
**per-gateway mTLS** で接続している（`docs/oss-gateway-bridge-infra.md`）。同じ仕組みを REST にも適用する。
