...
1 枚にまとめます。設計の全体像は [gateway-integration.md](gateway-integration.md)、デプロイ配置は
...
- [gateway-integration.md](gateway-integration.md) — 接続モデル（ingress/egress・point list 同期・mTLS §4）
- [oss-gateway-bridge-infra.md](oss-gateway-bridge-infra.md) — Traefik/mTLS/cert-manager/ArgoCD 配線（HITL）
- [oss-gateway-pointlist-sync.md](oss-gateway-pointlist-sync.md) — pointlist 同期 API の認証・信頼境界
