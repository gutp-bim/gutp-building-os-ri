...
> Issue #25. Related: `docs/oss-nats-design.md` (the NATS core), `docs/hono-device-test-plan.md`
> (device-side test plan), `docs/edge-device-mqtt-cutover.md` (cutover runbook).
...
based, `docs/oss-nats-design.md`); MQTT/AMQP are for devices that do not speak that path.
...
(`docs/hono-device-test-plan.md`) is executed.
...
- [ ] **Dual-connect and rollback procedures are tested** against a live edge device (requires
      execution of `docs/hono-device-test-plan.md`).
