...
## 残タスク（本書で live なのはこの 1 件のみ）

- **B-8 🔶** 管理系 API の Swagger 統合。コントローラへの `ProducesResponseType` 注釈は追加済みだが、
  `web-client/src/lib/admin/`（bespoke fetch）の生成クライアント置換は未完（CLAUDE.md に follow-up と明記）。
...
### B-8. 管理系 API の Swagger 統合 ★★ / L — 🔶 一部実装（残タスク）

> 状態: 管理系コントローラへ `ProducesResponseType` 注釈は追加済み。ただし `web-client/src/lib/admin/`
> （bespoke fetch）の生成クライアント置換は未完（CLAUDE.md に follow-up と明記）。
...
> この着手順は抽出当時の計画。実際には下表の順で概ね実装され、A 系・B 系ともほぼ完了した（残は B-8 の一部のみ）。
...
| 6 | A-4 / A-7 / B-4 以降 | 上記が安定してから順次 | ✅（残は B-8 の一部 🔶） |
