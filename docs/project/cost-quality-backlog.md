...
>
> **サマリ: 19 項目中 17 完了 ✅ / 1 一部 🔶（B-8）/ 1 未 ⬜（A-9）。**
>
> | 記号 | 意味 |
> |---|---|
> | ✅ | 実装済み（証跡を各項目に付記） |
...
## 残タスク（本書で live なのはこの 2 件のみ）

- **A-9 ⬜** Prometheus `scrape_interval` の見直し（現状 `observability/prometheus/prometheus.yml` は `15s` のまま）。軽微・任意。
...
### A-9. Prometheus スクレイプ間隔・保持の見直し ★ / S — ⬜ 未着手（残タスク）

> 状態: `observability/prometheus/prometheus.yml` は現状 `scrape_interval: 15s` のまま。軽微・任意。
...
> この着手順は抽出当時の計画。実際には下表の順で概ね実装され、A-9 を除き完了した。
...
| 6 | A-4 / A-7 / B-4 以降 | 上記が安定してから順次 | ✅（A-9 のみ ⬜） |
