# コスト最適化・品質改善バックログ（アーカイブ）

> **状態: アーカイブ（2026-07 完了棚卸し）。** 本書は OSS 公開前レビュー
> （`docs/project/oss-readiness-review.md`）の後続として抽出した棚卸しで、抽出時点では未実装だった。
> その後の v1.0.0-rc 開発でほぼ全項目が実装済みとなったため、**各項目に実装状況を付記して履歴として保存**する。
> **live なバックログ追跡は GitHub Issues が正**（本書は再オープンせず、残項目のみ下記「残タスク」を参照）。
>
> **サマリ: 19 項目中 19 完了 ✅。**（本書の全項目が実装済み。live なバックログ追跡は GitHub Issues が正。）
>
> | 記号 | 意味 |
> |---|---|
> | ✅ | 実装済み（証跡を各項目に付記） |
...
> 工数: S = 半日以内 / M = 1〜2日 / L = 3日以上。（優先度/工数は抽出当時の見積り、履歴として保持）

## 残タスク

本書の 19 項目はすべて実装済み（残タスクなし）。以降の cost/quality 改善は GitHub Issues で追跡する。

---

## A. コスト最適化

### A-1. OTel トレースが 100% サンプリング ★★★ / S — ✅ 実装済み

> 実装: `OtelSetup.cs` にサンプラー設定（`SetSampler`）を追加、環境変数で制御可能。
...
### A-2. API Server Dockerfile のレイヤーキャッシュ破綻 ★★★ / S — ✅ 実装済み

> 実装: `Dockerfile` は `COPY *.csproj → dotnet restore → COPY 残り` の正攻法に修正済み。
...
### A-3. `dotnet restore/publish` がソリューション全体を対象 ★★ / S — ✅ 実装済み

> 実装: `dotnet restore`/`dotnet publish` は `BuildingOS.ApiServer.csproj` を明示指定（依存プロジェクトのみ辿る）。
...
### A-4. harbor-push が変更有無に関わらず 3 イメージを毎回ビルド ★★ / M — ✅ 実装済み

> 実装: `harbor-push.yml` は `dorny/paths-filter@v4` で dotnet/proto → バックエンド、web-client → フロントのみビルド。
...
### A-5. ランタイムイメージの軽量化 ★★ / M — ✅ 実装済み

> 実装: ランタイムを `mcr.microsoft.com/dotnet/aspnet:8.0-noble-chiseled`（非 root・シェルなし）へ切替済み。
...
### A-6. Parquet レイクの既定が「無制限保持」 ★★ / S — ✅ 実装済み

> 実装: `LAKE_RETENTION_DAYS`（S3/MinIO ILM）を配線、`PARQUET_STREAM_MAX_BYTES` / `PARQUET_QUERY_MAX_FILES` を環境変数化（CLAUDE.md に記載）。
...
### A-7. ローカル開発スタックの「lite プロファイル」 ★★ / M — ✅ 実装済み

> 実装: 可観測性系を `--profile observability` に括り出し、既定は最小構成（CLAUDE.md「Observability … NOT in the base stack (A-7)」）。
...
### A-8. oss-ci の細かな無駄 ★ / S — ✅ 実装済み

> 実装: `actions/setup-dotnet` に `cache: true`、EF 等ツールは `dotnet tool restore`（ローカルツールマニフェスト）でキャッシュ対象化。
...
### A-9. Prometheus スクレイプ間隔・保持の見直し ★ / S — ✅ 実装済み

> 実装: compose がマウントする `oss-stack/prometheus/prometheus.yml` は既に `scrape_interval: 30s`。
> 併せてスタンドアロンの `docker-compose.observability.yml` が使う `observability/prometheus/prometheus.yml`
> も `30s` に整合（両 observability 経路を一致）。Tempo `block_retention` は既に明示値 `1h`（コスト配慮済み）。
...
### B-1. oss-ci が `Shared.Test` しか実行していない ★★★ / S — ✅ 実装済み

> 実装: `dotnet test BuildingOS.sln`（全ユニットプロジェクト）+ フロント `yarn test`（Frontend Check）で CI 実行。
...
### B-2. 外部 PR への軽量自動ゲート ★★★ / M — ✅ 実装済み

> 実装: `.github/workflows/pr-check.yml`（`pull_request` トリガー + paths filter + concurrency cancel-in-progress）。
...
### B-3. 依存関係の自動更新とセキュリティスキャン ★★★ / S — ✅ 実装済み

> 実装: `.github/dependabot.yml`（grouped）+ `codeql.yml`（js-ts / C#）導入済み。
...
### B-4. `Directory.Build.props` によるビルド設定の一元化 ★★ / S — ✅ 実装済み

> 実装: `DotNet/Directory.Build.props` を新設（共通ビルド設定を集約）。
...
### B-5. カバレッジ計測とレポート ★★ / S — ✅ 実装済み

> 実装: oss-ci でカバレッジ収集を有効化。
...
### B-6. 統合テスト・golden テストの定期実行 ★★ / S — ✅ 実装済み

> 実装: `integration-tests` weekly / `demo-smoke` monthly の schedule + 失敗時 Issue 自動起票（CLAUDE.md に記載）。
...
### B-7. Swagger / Aspida 型の drift 検知 ★★ / M — ✅ 実装済み

> 実装: oss-ci に Swagger 生成 → Aspida drift チェックを追加。
...
### B-8. 管理系 API の Swagger 統合 ★★ / L — ✅ 実装済み

> 実装: 管理系コントローラ（`UsersController` / `GroupsController` / `PermissionsController`）は
> `ProducesResponseType` 注釈込みで Swagger に含まれ（`docs/schema/swagger.yaml` に
> `/api/Users` 系・`/api/Groups` 系・`/api/Permissions/resolve` を出力）、Aspida 型を生成済み。
> `web-client/src/lib/admin/fetch-{users,groups,permissions,hierarchy}.ts` は生成クライアント
> （`apiClient().api.Users…` 等）上の薄い façade に置換済み（#38 / #143）。bespoke fetch
> （`src/lib/admin/http.ts`）の残る利用は resource / telemetry / control-audit の各 repository のみで、
> 管理系 API は非依存。

（レビュー #15 の再掲・当時）`/api/Users` / `/api/Groups` / `/api/Permissions` が Swagger 生成外で、
web-client は bespoke fetch（`src/lib/admin/http.ts`）を使用していた。型安全性と drift 検知（B-7）の
恩恵を受けられていなかった。
...
### B-9. EF マイグレーション drift チェックの実効化 ★ / S — ✅ 実装済み

> 実装: oss-ci の `dotnet ef migrations has-pending-model-changes` から `continue-on-error` を撤去（drift で赤くなる）。
...
### B-10. OSS ガバナンス定型の整備 ★ / S〜M — ✅ 実装済み

> 実装: `CODE_OF_CONDUCT.md` / `CODEOWNERS` / `.github/ISSUE_TEMPLATE/`（bug/feature/config）/
> `pull_request_template.md` / 英語ドキュメント（`docs/index-en.html`）を整備。CHANGELOG 自動化は任意の残余。
...
> この着手順は抽出当時の計画。実際には下表の順で概ね実装され、A 系・B 系とも全項目が完了した。
...
| 6 | A-4 / A-7 / B-4 以降 | 上記が安定してから順次 | ✅（全項目完了） |
