...
`docs/reference/oss-tech-stack-analysis.md` に基づき、Building OS を Azure マネージドサービス非依存（.NET 8 は維持）の OSS スタックへ段階的に移行する計画。
...
- 成果物: `docs/architecture/oss-timescaledb-schema.md`, `DotNet/BuildingOS.Shared/Migrations/Timescale/V001__telemetry_hypertable.sql`
...
- ADT クエリ → SPARQL マッピング表（成果物: `docs/architecture/oss-sparql-mapping.md`）:
