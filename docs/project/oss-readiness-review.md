...
| `DotNet/BuildingOS.ApiServer/Controllers/GatewayProvisioningController.cs` | `X-Gateway-Id` 信頼ヘッダの検証実装が docs/architecture/oss-gateway-pointlist-sync.md の設計どおりかの確認（trust boundary）。 |
...
| `docs/operations/oss-gateway-security-ops.md` | mTLS 運用手順が Traefik 設定例と整合しているか。 |
