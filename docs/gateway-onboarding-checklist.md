...
[oss-gateway-pointlist-sync.md](oss-gateway-pointlist-sync.md) に散在する手順を1枚に集約したもので、
...
- [ ] **4. ゲートウェイ側で Point List 同期を配線する**
  ...
  詳細: [oss-gateway-pointlist-sync.md](oss-gateway-pointlist-sync.md)(API 仕様・mTLS 信頼ヘッダ・
  検証チェックリスト)。
