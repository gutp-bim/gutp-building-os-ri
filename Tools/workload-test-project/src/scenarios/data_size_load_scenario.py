...
from src.core.config import DataSizeLoadConfig, DataSizeLoadStep
...
class DataSizeLoadScenario:
    ...
    def __init__(self, config: DataSizeLoadConfig, device_factory: DeviceFactory,
    ...
    async def execute_step(self, step_id: int) -> Dict[str, Any]:
        ...
        self.logger.info(f"Executing data size load step {step_id}")
        ...
        try:
            ...
            # データサイズ負荷テスト実行
            await self._execute_data_size_test(step_id, step_config)
        ...
        result = {
            ...
            'data_point_count': step_config.point_count,
    ...
    async def _create_devices(self):
        """テスト用デバイス作成（共有接続使用）"""
        total_devices = self.config.devices_per_type * len(self.config.device_types)
        self.logger.info(f"Creating {total_devices} virtual devices for data size load test (using shared connections)")
        ...
        for i, device_type in enumerate(self.config.device_types):
            ...
            # 各デバイスタイプに1つの共有接続を作成
            primary_device_id = f"dataload-{device_type}-primary"
            ...
            # 共有接続を使用する仮想デバイスを作成
            id_prefix = f"dataload-{device_type}"
    ...
    async def _execute_data_size_test(self, step_id: int, step_config: DataSizeLoadStep):
        """データサイズ負荷テスト実行"""
        if self.dry_run:
            self.logger.info(f"[DRY RUN] Would execute data size test with "
                           f"{step_config.point_count} points for "
        ...
        # 新しいデータポイント数でデバイスタスク開始
        self.logger.info(f"Starting virtual devices with {step_config.point_count} data points per message")
        
        for device in self.virtual_devices:
            task = asyncio.create_task(
                device.run_continuous(
                    interval_seconds=self.config.message_interval_seconds,
                    point_count=step_config.point_count,
                    duration_minutes=step_config.duration_minutes,
    ...
    async def _record_step_metrics(self, step_id: int, step_config: DataSizeLoadStep):
        """ステップメトリクス記録"""
        metrics = {
            'step_id': step_id,
    ...
    def _get_step_config(self, step_id: int) -> Optional[DataSizeLoadStep]:
        """ステップ設定取得"""
    ...
    async def cleanup(self):
        """リソースクリーンアップ"""
        self.logger.info("Starting data size load scenario cleanup")
        ...
        self.logger.info("Data size load scenario cleanup completed")