...
class IoTEdgeDevice:
    ...
    async def method_request_handler(self, method_request):
        ...
        # すべてのメソッドに対して200を返す
        payload = {"result": "success", "message": f"Method '{method_request.name}' executed successfully"}
        status = 200
    ...
    async def run(self):
        ...
        try:
            await self.connect()
            logger.info("エッジデバイスが起動しました。ダイレクトメソッドを待機中...")
            
            # 無限ループで接続を維持
            while True:
                await asyncio.sleep(10)  # 10秒間隔でスリープ
                logger.info("デバイス稼働中... (Ctrl+Cで終了)")
...
async def main():
    ...
    if not connection_string:
        ...
        logger.error("Dockerコンテナ実行時に -e オプションで設定してください")
        return
    
    # IoTエッジデバイスを作成・実行
    device = IoTEdgeDevice(connection_string)
    await device.run()
