...
public readonly partial struct Dtdl
{
    ...
    public readonly partial struct EnumValue
    {
        private ValidationContext ValidateObject(JsonValueKind valueKind, in ValidationContext validationContext, ValidationLevel level)
        {
            ...
            int propertyCount = 0;
            bool foundEnumValueValue = false;
            ...
            foreach (JsonObjectProperty property in this.EnumerateObject())
            {
                if (__TryGetCorvusLocalPropertiesValidator(property, this.HasJsonElementBacking, out ObjectPropertyValidator? propertyValidator))
                {
                    ...
                    if ((this.HasJsonElementBacking && property.NameEquals(JsonPropertyNames.EnumValueValueUtf8)) || (!this.HasJsonElementBacking && property.NameEquals(JsonPropertyNames.EnumValueValue)))
                    {
                        foundEnumValueValue = true;
                    }
            ...
            if (!foundEnumValueValue)
            {
                if (level >= ValidationLevel.Detailed)
                {
                    result = result.WithResult(isValid: false, $"6.5.3. required - required property \"enumValue\" not present.");
                }
                else if (level >= ValidationLevel.Basic)
                {
