...
| `HEALTH_INVENTORY_TTL_SEC` | #452 データ健全性: `GET /api/telemetry/health` は 1 リクエストで建物 1 棟ぶんの Point 台帳を読むため、**認可前**の twin スナップショット (`PointDetailInventoryCache`) をこの秒数だけ保持して OxiGraph への全件 SPARQL 連打を防ぐ。キャッシュされるのは認可前データだけで、読み取り認可の絞り込みは `AuthorizedTwinView.ListPointDetailsAsync` がリクエストごとに必ず再適用する。`0` 以下・数値以外は既定値に落ちる。 | `60` |
...
|----------|-------------|
| `WORKER_ROLE` | Which capability group this process registers (#400) — it selects which of the rows below apply. `all`\|`ingest`\|`lake`\|`control`; unset/blank/`all` = every capability in one process (the OSS RI default, behaviour unchanged from before #400). `ingest` = gRPC GatewayIngress + MQTT/Hono ingress + the `raw.*` normalizers; `lake` = Parquet writer + compaction + retention (or `ColdExportWorker` in timescale mode); `control` = `NatsPointControlWorker` + its binding handlers. **Unknown values fail fast at startup** (unlike `WARM_STORE`, which folds anything but `timescale` into parquet) — a typo silently meaning "all" would start the twin seed and compactor on every replica of a split deployment. **Replicas:** `ingest` and `control` are horizontally scalable; `lake` and `all` are deployed single-replica — **by preference, not because a second replica corrupts anything (#447)**. There is no leader lock and none is needed: `ParquetLakeWriterWorker` consumes a shared durable consumer, `LakeRetentionHostedService` PUTs an ILM rule derived from `LAKE_RETENTION_DAYS` alone under a fixed rule id (so concurrent applies converge on one rule), and `CompactionWorker` writes a *deterministic* compact key from a union of the hour's objects — two passes over the same listing produce the same content, and a pass whose planned sources were deleted underneath it (the one interleaving that would overwrite a complete object with a subset, in the limit an empty one) abandons the target and re-plans next cycle, counted as `building_os.compaction.skipped`. `all` additionally runs `OxiGraphSeedHostedService`, whose clear-and-rebuild is one transactional SPARQL UPDATE, so a concurrent seed of the same file converges too and never exposes a half-cleared twin (two replicas pointed at *different* seed files would overwrite each other — a misconfiguration, not a race). What a second `lake`/`all` replica actually buys is nothing: it re-reads and re-writes the same building-hours and throws one result away, so a steady non-zero `compaction.skipped` is the signal that someone scaled a role that does not benefit from it (#401 found no throughput win from splitting either). `docker-compose.roles.yaml` is the worked split example. **Readiness (#399):** `/health/ready` reports on the dependencies the selected capability set actually needs — NATS always, the twin (OxiGraph) when the role runs the twin client (`all`/`ingest`/`control`), the object store (MinIO/S3) when the role runs the lake (`all`/`lake`, and only in a mode that has one). Only a dependency whose loss stops the role's *only* job **gates** readiness (Unhealthy → 503): NATS everywhere, and the object store on `lake`. Everything else **signals** (Degraded → 200, so a plain `curl -sf` still passes): the object store on `all` (that process is also ingest+control), and the twin on every role — ingest/control read the twin through TTL caches (`PointMetadataCache` serves stale-while-revalidate) and keep working across an OxiGraph blip, so a 503 would evict every ingest replica at once for a degradation the caches already absorb. **A Degraded worker still reads as `up` in `GET /api/system/status`** (the fan-out collapses any 2xx to up) — read the `oxigraph` / `minio` rows next to it, or the worker's `/health/ready` body. |
...
| `HEALTH_PORT` | Internal HTTP/1.1 health surface port (default `8081`). Always on (the worker is always a WebApplication): `/health/live` (liveness = process serves HTTP, no checks), `/health/ready` + `/health` (readiness, **role-dependent since #399** — see the `WORKER_ROLE` row). Consumed by the system-status fan-out (#144) and orchestrator/compose probes. Does **not** open the external gRPC ingest surface. |
...
| `GRPC_INGRESS_REQUIRE_HIERARCHY` | #292 hierarchy policy: truthy (`true`/`1`/`yes`/`on`) rejects any ingress frame whose point the twin does not place under a building/device (skip + metric `result=no_building_path`/`no_device_link`); the effective state is logged at startup. **"Under a building" means the same thing here as in the #291 import-time orphan check: reachable from the owning `EquipmentExt` via the spatial chain (`locatedIn`→`Room`→`Level`→`Building`) *or* the `sbco:floor` literal join — either path counts.** It is deliberately *not* the denormalized `sbco:building` literal on the point: nothing joins that string, so it can be absent from a point a building contains (a Room-less twin, which is valid here) and present on a point no building contains. The literal remains the enrichment value published as the telemetry `building` field and the Parquet lake partition key. Reachability is resolved in the same bulk metadata load that backs the TTL cache (`IPointMetadataCache`), so the check costs no extra query on the hot path. **Default off** — a twin is routinely seeded before its hierarchy is complete (#118) and strict-by-default would drop every unplaced point; turn it on once the twin is fully modelled. Only used when `GRPC_INGRESS_PORT` is set. |
